...
function extractFileIds(msg) {
  ...
  if (msg.document)
    ids.push({ fileId: msg.document.file_id, type: "document" });
...
function storeMediaMessage(msg) {
  ...
  chatMessageStore[chatId] = chatMessageStore[chatId].filter(
    (m) => m.date > cutoff,
  );
}
...
bot.on("message", async (msg) => {
  ...
  console.log(
    `[${groupName}] ${msg.from?.username || msg.from?.first_name || "unknown"}: ${text || "[media]"}`,
  );
  ...
  if (msg.reply_to_message) {
    ...
    if (repliedFiles.length > 0) {
      ...
      const alreadyStored = chatStore.find(
        (m) => m.messageId === repliedMsg.message_id,
      );
      if (!alreadyStored) {
        if (!chatMessageStore[String(chatId)])
          chatMessageStore[String(chatId)] = [];
